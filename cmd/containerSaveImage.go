...
func correctContainerDockerConfigEnvVar(config *containerSaveImageOptions, utils piperutils.FileUtils) error {
	...
	log.Entry().Infof("config.ContainerRegistryURL: %v", config.ContainerRegistryURL)
	log.Entry().Infof("config.ContainerRegistryUser: %v", config.ContainerRegistryUser)
	log.Entry().Infof("config.ContainerRegistryPassword: %v", config.ContainerRegistryPassword)
	...
	// content, err := os.ReadFile(dockerConfigFile)
	// if err != nil {
	// 	log.Entry().Warningf("failed to read: %v", err)
	// }
	// fileContent := string(content)
	// log.Entry().Info("Content of the file: %v", fileContent)
